...
export default function CollectionsGrid({ selectedCategory, onSelectCategory, onViewDetails }) {
  ...
  const filteredProducts = useMemo(() => {
    return PRODUCTS.filter(product => {
      ...
      if (activeCategory !== "ALL") {
        if (product.category.toUpperCase() !== activeCategory.toUpperCase()) {
          return false;
        }
      }
      ...
      if (selectedMetal !== "ALL METALS") {
        if (selectedMetal === "22K GOLD" && !product.purity.includes("22K")) return false;
        if (selectedMetal === "POLKI" && !product.metal.toUpperCase().includes("POLKI")) return false;
        if (selectedMetal === "KUNDAN" && !product.metal.toUpperCase().includes("KUNDAN")) return false;
      }
      ...
      if (sortBy === "weight-low") return parseFloat(a.weight) - parseFloat(b.weight);
  ...
  const getCategoryCount = (catName) => {
    ...
    return PRODUCTS.filter(p => p.category.toUpperCase() === catName.toUpperCase()).length;
  };
